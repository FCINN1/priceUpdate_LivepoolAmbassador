...
async function main() {
  try {
    ...
    const LA_LIST = await playerSearch([236], 0); // playerSearch(시즌넘버, 최소오버롤)
    let LA_RESULTS = await playerPriceValue(LA_LIST, [1, 2, 3, 4, 5, 6, 7, 8]); // playerPriceValue(데이터 , 강화등급)
    await saveToDB(LA_RESULTS);
